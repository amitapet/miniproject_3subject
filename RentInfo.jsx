...
function RentInfo() {
  ...
  if (window.confirm("คุณต้องการยกเลิกการจองนี้หรือไม่?")) {
    try {
      await fetch(`http://localhost:3000/reserve/cancel/${id}`, { method: "POST" });
  ...
  const stations = Array.from(new Set(bookings.flatMap((b) => [b.origin, b.destination])));
                ...
                <tr key={b.id}>
                  ...
                  <td>
                    <div className="action-buttons">
                      {b.status !== "ยกเลิก" && (
                        <>
                          <button className="ticket-btn" onClick={() => handleTicket(b)}>ตั๋ว</button>
                          <button className="cancel-btn" onClick={() => handleCancel(b.id)}>ยกเลิก</button>
                        </>
                      )}
                    </div>
                  </td>
